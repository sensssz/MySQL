...
static uint build_bitmap_for_nested_joins(List<TABLE_LIST> *join_list,
        uint first_unused);
static ORDER *my_remove_const(JOIN *join,ORDER *first_order, Item *cond,
...
JOIN::optimize()
{
    ...
    */
    ...
    statistics = make_join_statistics(this, select_lex->leaf_tables, conds, &keyuse,
                                           first_optimization);
    TraceTool::path_count--;
    if (statistics)
    {
    ...
    */
    ...
    /* Optimize distinct away if possible */
    {
        ...
        order= ORDER_with_src(my_remove_const(this, order, conds, 1, &simple_order, "ORDER BY"), order.src);;
        if (thd->is_error())
        {
            error= 1;
    ...
    */
    ...
    simple_group= 0;
    {
        ...
        group_list= ORDER_with_src(my_remove_const(this, group_list, conds,
                                                rollup.state == ROLLUP::STATE_NONE,
                     ...
                     Key_use_array *keyuse_array, bool first_optimization)
{
    ...
    */
            ...
            p_pos++)
    {
        ...
        tmp=join_read_const_table(s, p_pos);
        if (tmp)
        {
            if (tmp > 0) {
            ...
            if (s->dependent)				// If dependent on some table
            {
                ...
                */
                ...
                {   // system table
                    ...
                    join->const_table_map|=table->map;
                    set_position(join, const_count++, s, NULL);
                    tmp= join_read_const_table(s, join->positions+const_count-1);
                    if (tmp)
                    {
                        if (tmp > 0) {
                            goto error;    // Fatal error
...
static ORDER *
my_remove_const(JOIN *join,ORDER *first_order, Item *cond,
