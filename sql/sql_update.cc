 ...
 */
void read_and_update(THD *thd, TABLE_LIST *table_list, List<Item> &fields, List<Item> &values, ha_rows limit,
                     bool ignore, bool using_limit, bool will_batch, int res, uint &dup_key_found, TABLE *table,
                     SQL_SELECT *select, COPY_INFO &update, int &error, ha_rows &updated, ha_rows &found,
                     READ_RECORD &info);
...
int read_record(READ_RECORD *info)
{
  int result;
  TraceTool::path_count++;
  result = info->read_record(info);
  TraceTool::path_count--;
  return result;
}

int read_and_update(THD *thd,
                    TABLE_LIST *table_list,
                    List<Item> &fields,
                    List<Item> &values,
                    ha_rows limit,
                    bool ignore,
                    bool using_limit, bool will_batch,
                    TABLE *table,
                    SQL_SELECT *select,
                    COPY_INFO &update,
                    ha_rows &updated, ha_rows &found,
                    READ_RECORD &info)
{
  int res;
  uint dup_key_found;
  ...
  while (!(error=read_record(&info)) && !thd->killed)
  {
                 ...
                 ha_rows *found_return, ha_rows *updated_return)
{
  ...
  error = read_and_update(thd, table_list, fields, values, limit, ignore, using_limit,
                          will_batch, table, select, update, updated, found, info);
